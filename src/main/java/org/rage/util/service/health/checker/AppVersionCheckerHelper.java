...
/**
 * Check if the implementation version is the same as the passed why
 * */
public class AppVersionCheckerHelper {
	 ...
	 * @param project
	 * */
	public static void checkVersion(ProjectExtended project){
		if(project != null && project.isStatus() && !project.getAttributes().isEmpty()){
			if(project.getAttributes().containsKey(IMPLEMENTATION_VERSION) 
					&& project.getAttributes().get(IMPLEMENTATION_VERSION) != null && project.getTargetVersion() != null &&
